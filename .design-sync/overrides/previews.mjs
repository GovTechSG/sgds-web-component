  ...
  for (const c of components) {
    ...
    try {
      await build({
        ...
        // FORK: pin a browser target and use lit's decorator mode. The stock
        // build sets neither. Two SGDS story problems it fixes:
        //  1. target unset → esbuild defaults to `esnext` and PRESERVES
        //     decorator syntax; no browser parses it → the preview module
        //     throws "Invalid or unexpected token" at load. es2020 downlevels.
        //  2. Some story mocks are lit elements using field decorators
        //     (@property()) — legacy/experimental decorators. esbuild's
        //     standard-decorator mode rejects them ("Unsupported decorator
        //     location: field"). Parsing story .js as TS with
        //     experimentalDecorators (+ useDefineForClassFields:false, the lit
        //     requirement) applies the correct legacy transform for both class
        //     (@customElement) and field (@property/@state) decorators.
        target: 'es2020',
        tsconfigRaw: { compilerOptions: { experimentalDecorators: true, useDefineForClassFields: false } },
        nodePaths: NODE_MODULES ? [NODE_MODULES] : undefined,
        plugins,
        loader: { '.js': 'ts', '.jsx': 'tsx', ...(loaders ?? {}) },
