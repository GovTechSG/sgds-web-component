...
export function generatePreviewSource(c, opts) {
  ...
  for (const s of paired) {
    ...
    s.emitted = label;
    lines.push(`export const ${label} = /* ${s.name} */ __bridge(compose(${mod}, ${JSON.stringify(s.exportKey)}));`);
  }
...
// SGDS ships lit-html web-component stories: a story's render() returns a lit
// TemplateResult, which React's mount (emit.mjs, ReactDOM.createRoot().render)
// cannot render directly. __bridge wraps each composed story in a React
// component that renders the lit output into its own host div via lit's own
// render(). The custom elements (<sgds-*>) are registered by the shipped
// bundle (window.<GLOBAL>) that the preview html loads first, and 'lit' bundles
// into this same preview compile, so the story's html and this render() share
// one lit instance. TemplateResult | string | number | Node | array | nullish
// are all valid render() inputs, so no per-kind branching is needed.
const LIT_BRIDGE = `function __bridge(fn: any) {
