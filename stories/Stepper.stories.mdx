...
import "../lib/Stepper";
import { SgdsCheckbox } from "../lib/Stepper";
...
<Meta
  ...
  argTypes={{
    steps: {
      control: "object",
      description: "Array of objects that can be passed in with properties of `title` and `stepHeader` as follows:",
      table: {
        category: "props",
        type: {
          summary: `
    ...
    methodType: {
      control: "disable",
      description:
        "prop that holds value to invoke the stepper methods such as: `decrement`, `increment`, `first` and `last`. To be used with stepperId.",
      table: {
