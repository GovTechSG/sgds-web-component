...
- Use `primary` badge to additional information or context that might be helpful to the user but does not require immediate attention. Commonly used for non-urgent updates, background details, or status labels.
...
- Use `neutral` badge to provide general information or categorisation that does not require immediate attention or imply any urgency. It typically has a muted or grey colour to avoid drawing too much focus.
...
A dismissible badge is used to display temporary information or alerts that users can remove once they have acknowledged or no longer need. It helps keep the interface clean and gives users control over what content remains visible.
...
A badge with a leading icon conveys the purpose or status of the badge through a visual symbol, enhancing recognition and clarity.
