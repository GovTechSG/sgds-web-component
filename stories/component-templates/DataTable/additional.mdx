...
When `serverSort` is enabled, clicking a sort header emits the `sgds-sort` event instead of sorting rows locally.
The event detail contains the `key` (from `sortKey`) and the `direction` (`ascending`, `descending`, or `none`).
