...
The preferred way to use `sgds-stepper` is by composing it with `<sgds-step>` child components. This approach provides better flexibility and is the recommended pattern going forward.

```tsx
export interface SgdsStepProperties {
  stepHeader: string; // The header text for the step
  iconName?: string; // Optional icon name from SGDS icon set
...
See a <a target="_blank" href="https://stackblitz.com/github/clukhei/sgds-wc-react-stepper?file=src%2FApp.tsx">working example of stepper in a React app</a>
...
See a <a target="_blank" href="https://stackblitz.com/github/tonghauhive/sgds-vue-stepper?file=src%2Fcomponents%2FStepper.vue">working example of stepper in a Vue app</a>
...
We accept contributions from the community!
If you have an SgdsStepper to showcase from a different framework or a better example to showcase,
raise a PR or raise an issue with your StackBlitz example on our <a href="https://github.com/GovTechSG/sgds-web-component" target="_blank">GitHub page</a>.
If the examples above are not working, notify us by raising a GitHub issue.

> > > > > > > 3848155d2f6a2749d56538b3359c4f4a412b81c9
