...
Use the `action` slot to place a call-to-action button inside a filled alert. Pass `<sgds-button>` with `variant="outline"` and `size="sm"`. Use `tone="fixed-light"` for all variants except `warning`, which uses `tone="fixed-dark"` to maintain contrast against the light background.
...
For outlined alerts, use `<sgds-button>` with `variant="outline"`, `size="sm"`, and `tone="neutral"`.
