...
describe("sgds-stepper", () => {
  ...
  it("should have the is-active class on step 1 when activeStep set to 0", async () => {
    const el = await fixture(html`
      <sgds-stepper activeStep="0"></sgds-stepper>
    `);

    expect(
      el.shadowRoot?.children[0].querySelector(".stepper-item")?.classList.value
    ).to.contain("is-active");
  });

  it("when activeStep set to 2, should update the `is-active` to the previous step when clicked", async () => {
    const el = await fixture(html`
      <sgds-stepper activeStep="2"></sgds-stepper>
    `);

    const stepperItemTwo = el.shadowRoot?.querySelectorAll(
      ".stepper-item"
    )[1]! as SgdsStepper;
    const stepperItemThree = el.shadowRoot?.querySelectorAll(
      ".stepper-item"
    )[2]! as SgdsStepper;
...
describe("sgds-stepper, sgds-button interactions", () => {
  it("should increment the active step when the incrementStep() method is called", async () => {
    ...
    if (button) {
      expect(
        stepper.shadowRoot?.querySelectorAll(".stepper-item")[0]
      ).to.have.class("is-active");
      button.shadowRoot?.querySelector("button")?.click();
      await elementUpdated(el);
      expect(
        stepper.shadowRoot?.querySelectorAll(".stepper-item")[0]
      ).not.to.have.class("is-active");
      expect(
        stepper.shadowRoot?.querySelectorAll(".stepper-item")[1]
      ).to.have.class("is-active");
    }
  });
  ...
  it("should increment the active step when the incrementStep() method is called, showing the previous step 1 to have is-clickable, is-completed props", async () => {
    ...
    if (button) {
      ...
      expect(
        stepper.shadowRoot?.querySelectorAll(".stepper-item")[0]
      ).to.have.class("is-completed");
      expect(
        stepper.shadowRoot?.querySelectorAll(".stepper-item")[0]
      ).to.have.class("is-clickable");
      expect(
        stepper.shadowRoot?.querySelectorAll(".stepper-item")[0]
      ).not.to.have.class("is-active");
    }
  ...
  it("should decrement the active step when the decrementStep() method is called, showing the previous step 2 to not have is-clickable, is-completed props", async () => {
    ...
    if (button) {
      ...
      expect(
        stepper.shadowRoot?.querySelectorAll(".stepper-item")[1]
      ).not.to.have.class("is-completed");
      expect(
        stepper.shadowRoot?.querySelectorAll(".stepper-item")[1]
      ).not.to.have.class("is-clickable");
    }
  });
  ...
  it("should set the active step to the last step when the lastStep() method is called", async () => {
    const el = await fixture(html`
      <sgds-stepper id="myStepper" activeStep="0"></sgds-stepper>
      <sgds-button stepperId="myStepper" methodType="last" variant="primary"
        >Next</sgds-button
      >
    `);
    ...
    if (button) {
      ...
      expect(
        stepper.shadowRoot?.querySelectorAll(".stepper-item")[2]
      ).to.have.class("is-active");
    }
  });
  ...
  it("should set the active step to the first step when the firstStep() method is called", async () => {
    const el = await fixture(html`
      ...
      <sgds-button stepperId="myStepper" methodType="first" variant="primary"
        >Next</sgds-button
      >
    `);
