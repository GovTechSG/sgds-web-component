...
For full definitions of semantic categories (`primary`, `accent`, `success`, `danger`, `warning`, `purple`, `cyan`, `neutral`) and what each suffix modifier means (`default`, `emphasis`, `muted`, `fixed-light`, `fixed-dark`, `inverse`, `surface`, `alternate`, `overlay`, `translucent`, `transparent`), see **[`utilities-color-semantics`](../sgds-utilities-color-semantics/SKILL.md)**.
