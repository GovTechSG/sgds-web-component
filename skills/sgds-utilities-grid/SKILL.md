---
name: sgds-utilities-grid
description: "Use the SGDS grid system (.sgds-container, .sgds-grid, .sgds-col-*) for page and content layout. The SGDS grid classes are always preferred over generic Tailwind sgds:grid-cols-* utilities — they encode SGDS breakpoints, gutters, column counts, and container widths exactly. Apply this skill whenever a user asks about grid layout, columns, responsive layout, column spans, containers, or structuring SGDS pages."
metadata:
...
**Required**: Import the grid CSS from `@govtechsg/sgds-web-component/css/sgds.css`. The grid system is part of the base stylesheet — no extra import is needed beyond the standard SGDS CSS setup.
...
Use when content sits alongside a sticky sidebar (i.e. the sidebar layout from `sgds-pattern-layout`). Narrower max-widths preserve readable line lengths.
