 ...
 */
export class SgdsSidebarGroup extends SgdsElement {
   ...
   * @internal
   */
  @state() private selectedNestedLevel: SgdsSidebarGroup;
  ...
  private _handleKeyPress(event: KeyboardEvent) {
    if (event.key === "Enter") {
      ...
      this._handleClick(event.target as SgdsSidebarGroup);
      return;
    }
  }
  ...
  /**
   * Checks for nested sgds-sidebar-item elements as direct children.
  ...
  private checkNestedOptions() {
    const nestedGroup = this.querySelectorAll(":scope > sgds-sidebar-group");
    const nestedOptions = this.querySelectorAll(":scope > sgds-sidebar-item");
    console.log("nestedOptions", nestedOptions.length, "nestedGroup", nestedGroup.length);
    this.hasNestedOptions = nestedOptions.length > 0 || nestedGroup.length > 0;
  }
   ...
   */
  private _handleClick(element?: SgdsSidebarGroup) {
  ...
  render() {
    console.log("hasNestedOptions", this.hasNestedOptions);
