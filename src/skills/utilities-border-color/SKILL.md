...
**Related skills**:
- **[utilities-border-width](../utilities-border-width/SKILL.md)** — Border thickness and side selection (`sgds:border`, `sgds:border-2`, `sgds:border-l-4`, etc.)
- **[utilities-border-radius](../utilities-border-radius/SKILL.md)** — Rounded corners (`sgds:rounded-lg`, `sgds:rounded-full`, etc.)
...
For full definitions of semantic categories (`primary`, `accent`, `success`, `danger`, `warning`, `purple`, `cyan`, `neutral`) and what each suffix modifier means (`default`, `emphasis`, `muted`, `fixed-light`, `fixed-dark`), see **[`utilities-color-semantics`](../utilities-color-semantics/SKILL.md)**.
